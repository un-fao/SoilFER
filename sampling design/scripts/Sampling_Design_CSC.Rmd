...
packages_table <- data.frame(
  Package = c(
    ...
    "manipulate", "dplyr", "synoptReg", "doSNOW", "Rfast", "fields", "ggplot2", "rassta"
  ),
  Purpose = c(
...
Users can calculate the total number of sampling sites based on predefined values. For example,out of 300 total sites (i.e., samples), 70% are selected for analysis (`share`), as this percentage aligns with the proportion of croplands. The final number of samples is stored in `nsamples`. Adjusting the number of sites ensures that only a subset of the data is analyzed.

```{r number-sites, include=TRUE, warning=FALSE, message=FALSE}
