...
><p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> Ensure you have the `tinytex`, `knitr`, `rmarkdown`, and `rstudioapi` R packages installed to successfully run and render R Markdown documents before running this script.  
...
First, set the working directory. This ensures that all paths in the script are relative to the folder's location.
...
packages_table <- data.frame(
  Package = c(
    "sp", "terra", "raster", "sf", "sgsR", "entropy", "tripack", "tibble", "manipulate", 
    "dplyr", "synoptReg", "doSNOW", "Rfast", "fields", "ggplot2", "rassta", "snowfall"),
  Purpose = c(
...
><p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> This function is particularly useful for creating spatially balanced sampling designs by considering existing sampling points and covariate distributions. </p>
...
> <p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> This function ensures spatial consistency by sampling TSUs only within specified land-use areas, making it ideal for hierarchical sampling designs. </p>
...
> <p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> Always verify that all geospatial datasets use the same CRS before performing any spatial operations. Misaligned CRSs can lead to incorrect analyses. </p>
...
### Environmental covariates for selecting PSUs

The additional environmental covariates include information on non-protected areas, high slope mask, geology, and [geomorphology](https://code.earthengine.google.com/038b661e6a022e63ab46411df8ea688f). These inputs are optional and do not affect the selection of sampling units.
...
> <p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> There is an initial number of `r length(names(cov.dat))` environmental covariates. </p>
...
```
> <p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> The final number of soil climate variables is `r length(names(newhall))`. </p>
...
```
> <p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> Use the correct operation for your `npa` dataset type (shapefile or raster) to ensure compatibility and accurate processing. </p>
...
```
> <p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> The formula `sum(lu, na.rm = TRUE) * 100 / 400` ensures that the crop percentage is calculated accurately, with 400 being the total pixels in a 2x2 km PSU. </p>
...
```{r fig-landuse-coverage-psus, fig.align='center', include=TRUE, message=FALSE, echo=FALSE, fig.width=12, fig.height=10}
...
```
> <p style="font-size: 75%;">💡<span style="color: #000000;">**Why this matters**:</span> Cropping and resampling ensure that the spatial resolution and extent of covariates match the PSU grid, facilitating accurate spatial analysis and clustering. </p>

### Computing optimal sample size for the PSUs

After rasterising the PSUs (`PSU.r`) for Covariate Space Coverage, it is essential to calculate the optimal sample size for the sample design and future digital soil mapping based on divergence metrics. The method follows the publication: *Divergence metrics for determining optimal training sample size in digital soil mapping*  DOI: [10.1016/j.geoderma.2023.116553](https://doi.org/10.1016/j.geoderma.2023.116553)  
...
```{r computing-opt-sample-size, include=TRUE, message=FALSE, warning=FALSE, fig.width=12, fig.height=10}
...
### Covariate Space Coverage - Computing PSUs
...
```
> <p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> Users can follow the progress by observing the printed output. For example, "1 out of 10" indicates that 1 cluster interaction has been done, with 9 more left to process. </p>
...
```{r fig-target-psus, fig.align='center', include=TRUE, message=FALSE, echo=FALSE, fig.width=12, fig.height=10}
...
```{r fig-pc1-pc2, fig.align='center', include=TRUE, message=FALSE, echo=FALSE, fig.width=12, fig.height=10}
...
In the previous section, we computed PSUs. Now, the focus shifts to SSUs and TSUs, which add hierarchical levels to the sampling process. SSUs are 100m x 100m grid cells within each PSU, selected based on high-resolution environmental covariates derived from the Shuttle Radar Topography Mission (SRTM) digital elevation data and Sentinel-2 multispectral imagery. The Google Earth Engine script for terrain derivatives can be accessed [here](https://code.earthengine.google.com/038b661e6a022e63ab46411df8ea688f), while the script for Sentinel-2 multispectral imagery indices is available [here](https://code.earthengine.google.com/42e997378198c2cc5d06b98150d90e93). The selection process involves the reallocation of SSUs within each PSU using CSC Sampling to optimise spatial representativeness based on environmental heterogeneity. TSUs are randomly distributed points within each SSU, providing finer-scale sampling resolution. 
...
```{r loading-high-res-env-covars, include=TRUE, message=FALSE, warning=FALSE, fig.width=12, fig.height=10}
...
```
> <p style="font-size: 75%;">💡<span style="color: #000000;">**Note**:</span> Users can follow the progress by observing the printed output. For example, `"1 out of 30"` indicates that processing for the first PSU is complete, and there are 29 more PSUs left. </p>

Here, you can see how the SSUs are selected using CSC and the set of high-resolution remote sensing data.

```{r explan-clusters-ssus, fig.align='center', include=TRUE, message=FALSE, warning=FALSE, echo=FALSE, fig.width= 12, fig.height= 12}
...
```{r fig-target-tsus, fig.align='center', include=TRUE, message=FALSE, warning=FALSE, echo=FALSE, fig.width= 12, fig.height= 10}
...
```{r fig-alternative-tsus, fig.align='center', include=TRUE, message=FALSE, warning=FALSE, echo=FALSE, fig.width=12, fig.height=10}
